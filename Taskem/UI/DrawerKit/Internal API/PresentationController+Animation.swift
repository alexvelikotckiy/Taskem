...
extension DrawerPresentationController {
    func animateTransition(to endingState: DrawerState, animateAlongside: (() -> Void)? = nil, completion: (() -> Void)? = nil) {
        ...
        animator.addCompletion { endingPosition in
            ...
            // Change: set maxCornerRadius only when fullExpansionBehaviour covers full screen
            let shouldSetCornerRadiusToZero =
                ...
                (isEndingStateCollapsedOrFullyExpanded && self.configuration.fullExpansionBehaviour == .coversFullScreen)
    ...
    func positionsY(startingState: DrawerState,
