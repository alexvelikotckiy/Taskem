...
final class DrawerPresentationController: UIPresentationController {
    var configuration: DrawerConfiguration // intentionally internal and immutable
    ...
    func gestureAvailabilityConditionsDidChange() {
        // Change drawerDismissalTapGR availability
        switch targetDrawerState {
        case .fullyExpanded:
            if configuration.fullExpansionBehaviour == .coversFullScreen {
                drawerDismissalTapGR?.isEnabled = false
            } else {
                drawerDismissalTapGR?.isEnabled = true
            }
...
extension DrawerPresentationController: DrawerPresentationControlling {}
