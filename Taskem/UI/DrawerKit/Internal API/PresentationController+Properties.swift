...
extension DrawerPresentationController {
    var shouldBeginTransition: Bool {
        return (presentedViewController as? DrawerPresentable)?.shouldBeginTransitioning ?? true
    }
    ...
    var currentDrawerCornerRadius: CGFloat {
        ...
        set {
            ...
            (presentedViewController as? DrawerPresentable)?.drawerViewParticipants.forEach { $0.layer.cornerRadius = radius }
            if #available(iOS 11.0, *) {
                presentedView?.layer.maskedCorners = [.layerMinXMinYCorner, .layerMaxXMinYCorner]
                (presentedViewController as? DrawerPresentable)?.drawerViewParticipants.forEach { $0.layer.maskedCorners = [.layerMinXMinYCorner, .layerMaxXMinYCorner] }
            }
        }
    }
