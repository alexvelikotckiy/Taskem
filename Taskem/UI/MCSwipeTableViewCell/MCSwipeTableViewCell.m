...
CGFloat const kMCStop1                       = 0.25; // Percentage limit to trigger the first action
CGFloat const kMCStop2                       = 0.75; // Percentage limit to trigger the second action
CGFloat const kMCBounceAmplitude             = 20.0; // Maximum bounce amplitude when using the MCSwipeTableViewCellModeSwitch mode
CGFloat const kMCDamping                     = 1.0;  // Damping of the spring animation
CGFloat const kMCVelocity                    = 0.9;  // Velocity of the spring animation
CGFloat const kMCAnimationDuration           = 0.4;  // Duration of the animation
NSTimeInterval const kMCBounceDuration1      = 0.2;  // Duration of the first part of the bounce animation
NSTimeInterval const kMCBounceDuration2      = 0.1;  // Duration of the second part of the bounce animation
NSTimeInterval const kMCDurationLowLimit     = 0.25; // Lowest duration when swiping the cell because we try to simulate velocity
NSTimeInterval const kMCDurationHighLimit    = 0.1;  // Highest duration when swiping the cell because we try to simulate velocity
...
@property (nonatomic, strong) UIPanGestureRecognizer *helperGestureRecognizer;
...
- (void)removeHelperGestureRecognizer:(UIPanGestureRecognizer *)geture {
    [self removeGestureRecognizer:_helperGestureRecognizer];
}
...
- (void)handleWithGestureRecognizer:(UIPanGestureRecognizer *)gesture {
...
- (void)handlePanGestureRecognizer:(UIPanGestureRecognizer *)gesture {
    ...
    if (state == UIGestureRecognizerStateBegan || state == UIGestureRecognizerStateChanged) {
        ...
        if ([_delegate respondsToSelector:@selector(swipeTableViewCell:didSwipeWithGesture:)]) {
            [_delegate swipeTableViewCell:self didSwipeWithGesture:gesture];
        }
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if ([gestureRecognizer class] == [UIPanGestureRecognizer class]) {
        ...
        if ([_delegate respondsToSelector:@selector(swipeTableViewCellShouldSwipe:)]) {
            BOOL shouldSwipe = [_delegate swipeTableViewCellShouldSwipe:self];
            if (!shouldSwipe) {
                return NO;
            }
        }
...
- (void)moveWithDuration:(NSTimeInterval)duration andDirection:(MCSwipeTableViewCellDirection)direction {
    ...
    [UIView animateWithDuration:duration delay:0 options:(UIViewAnimationOptionCurveEaseOut | UIViewAnimationOptionAllowUserInteraction) animations:^{
        ...
        if ([self->_delegate respondsToSelector:@selector(swipeTableViewCellShouldExecuteCompletion:)]) {
            BOOL shouldExecute = [self->_delegate swipeTableViewCellShouldExecuteCompletion:self];
            // We notify the delegate that we just ended swiping.
            if ([self->_delegate respondsToSelector:@selector(swipeTableViewCellDidEndSwiping:)]) {
                [self->_delegate swipeTableViewCellDidEndSwiping:self];
            }
            if (shouldExecute) {
                [self executeCompletionBlock];
            }
